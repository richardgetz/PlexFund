...
def get_m3u8_rabbitstream(id):
    ...
    resp = get(
        "https://rabbitstream.net/ajax/embed-5/getSources",
        headers=headers,
        params=params,
    ).json()
    return resp
...
class StreamGrabber:
    ...
    def get_available_servers(self):
        if self.type == "show":
            ...
            for s in servers:
                clean_name = s.text.split("Server ")[-1].lower()
                self.available_servers[clean_name] = {
                    "id": s["data-id"],
                    "name": clean_name,
                }
    ...
    def get_file_link(self, name, link):
        if name.lower() == "streamlare":
            ...
            response = requests.post(
                ...
                headers={"referer": f"https://sltube.org/e/{id}"},
                data={"id": id},
            )
            data = response.json()
        ...
        if name.lower() == "vidcloud":
            ...
            response = requests.get(
                "https://rabbitstream.net/ajax/embed-5/getSources",
                headers={"referer": "https://www.2embed.to/",},
                params={
        ...
        if stream.available_servers.get(n.lower(), False):
            ...
            if file_url and type:
                if type.lower() in ["mp4", "mkv", "mov", "wmv", "avi"]:
                    download_file(
                        file_url, filename=f"{BASE_VIDEO_PATH}/name.of.show.SNNENN.ext"
                    )
                if type.lower() in ["m3u8", "hls"]:
                    # todo filename/path
                    m3u8_To_MP4.multithread_uri_download(
                        file_url,
                        customied_http_header={"referer": "https://www.2embed.to/",},
                    )
